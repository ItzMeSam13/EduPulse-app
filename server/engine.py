# engine.py — Drop in server/ folder
# CV window pops up on backend machine + streams metrics to frontend via FastAPI
...
# ── Shared state (FastAPI reads these) ────────────────────────────
metrics_queue = queue.Queue(maxsize=200)
_stop_flag    = threading.Event()
_thread       = None

# ── Optional DistractionScorer ────────────────────────────────────
try:
    from distraction.scorer import DistractionScorer
    _scorer   = DistractionScorer()
    USE_SCORER = True
    print("DistractionScorer loaded.")
except ImportError:
    _scorer    = None
    USE_SCORER = False
    print("DistractionScorer not found — phone/side-convo disabled.")
...
def _run():
    # Init models inside thread to avoid cross-thread issues
    ...
    print("EduPulse window opened. Press Q on window to stop.")
    ...
    while not _stop_flag.is_set():
        ...
        if USE_SCORER and frame_count % 3 == 0:
            try:
                ...
                distraction_result = _scorer.score(rgb_full)
            except Exception:
                pass
        ...
        if class_scores:
            ...
            # ── Push metrics to queue for frontend ────────────────
            try:
                ...
                pass  # Frontend too slow — just drop, don't block CV loop
...
def start():
    """Start the CV engine in a background thread."""
    global _thread
...
def stop():
    """Signal the CV engine to stop."""
    _stop_flag.set()
    print("Engine stop signal sent.")
